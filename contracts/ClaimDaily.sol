...
contract TokenVesting is Ownable {
    ...
    function fundVesting(uint256 _totalTokens) public onlyOwner {
        // require(token.allowance(msg.sender, address(this)) == _totalTokens, "Not allow spend token");
        totalTokens = totalTokens.add(_totalTokens);
        totalTokensDeposited = totalTokensDeposited.add(_totalTokens);
    ...
    function claimTokens() public {
        ...
        // user.tokensClaimed not equals 0
            ...
            require(tokensToClaim > 0,'Invalid amount');
    ...
    function claimAble(address _sender) external view returns(uint256) {
        uint256 amtToClaim;
        UserInfo storage user = userInfo[_sender];
        if (
            user.amount < user.tokensClaimed.add(1)
        ...
        if (timePassed < cliff) {
            ...
            uint256 _amount = user.amount.sub(firstClaim);
            amtToClaim = _amount.mul(time).div(totalPeriods).add(firstClaim).sub(user.tokensClaimed);
    ...
    function removeWhitelistUser(address _address) public onlyOwner {
      ...
      {  
        ...
        for(uint i = 0; i < addrs.length; i++){
            UserInfo storage user = userInfo[addrs[i]];
            user.amount = amounts[i];
            user.tokensClaimed = 0;
        }
      }
